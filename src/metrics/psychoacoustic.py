...
def db_to_phone(db, f):
    raise NotImplementedError()


def phone_to_db(ph, bk):
    raise NotImplementedError()
...
def bark_to_freq(x):
    """
    // this method of conversion comes from the edges of the critical bandwidth
    barkToFreq {var barkEdge = #[0, 100, 200, 300, 400, 510, 630, 770, 920, 1080, 1270, 1480,
            1720, 2000, 2320, 2700, 3150, 3700, 4400, 5300, 6400, 7700, 9500, 12000, 15500];
            ^barkEdge.blendAt(this)
    }

    // this one is the inverse of the Traunmuller approximation in asBark
    // differs from asBark below 220 hz
    barkToHz { ^1960 / (26.81 / (this + 0.53) - 1) }
    """
...
def freq_to_bark(x):
    ...
    asBark { var bk;
        if (this <= 219.5) {
            ...
            bk = ( (26.81 * this) / (1960 + this) ) - 0.53; // Traunmuller
//            bk = (26.81 / (1 + (1960 / this)) - 0.53); // alt
            if (bk > 20.1) { bk = bk + (0.22 * (bk-20.1)) }
        }
        ^bk
    }
    """
    return 26.81 / (1.0 + (1960.0 / float(x))) - 0.53
